...
function semicolonweb_loadScriptAsync(a, o) {
    ...
    (n.onload = o), (n.src = a), document.head.appendChild(n);
			...
			$("#form-reg").submit(function(e) 
			{
				...
				$.ajax({
						type: "POST",
						url: "https://marketing.newera.mx/promociondallas/php/app_registro.php",
						data: form.serialize(), 
						success: function(data)
						{
							...
							{	setCookie('promoJB-inicio', '', -100);
								$("#promo-submit").hide();
								$('#regModal').modal('hide');
								$(location).attr('href','trivia.php');
							}
							else if(respuesta=='existe')
							{	$(".existe").removeClass('d-none');
								console.log("Ticket ya registrado, por favor ingresa un nuevo ticket.");
			...
			$(['images/dinamica1.png','images/header.png','images/newera.png','images/promo.png','images/parallax.png','images/promo_mobile.png','images/registro.png','images/registro_mobile.jpg']).preload();
			...
			$(document).ready(function () {
			    ...
			    if (getCookie('promoJB_remember') != 'RememberAge') {
			      $('#ageModal').modal('show');
			      initAge();
			    } 
			    
			    function initAge() {
			      var month = 0; var day = 0; var year = 0;
			      $("#age-submit").on("click", function () {
			        age['month'] = $("#v-month").val(); age['day'] = $("#v-day").val(); age['year'] = $("#v-year").val();
			        checkDate();
			    ...
			    function checkDate() {
			      if (age.month == '' || age.day == '' || age.year == '') 
			      { if (age.month == '') { $("#v-month").css("border", "2px solid #f00");  }
			        if (age.day == '') { $("#v-day").css("border", "2px solid #f00"); }
			        if (age.year == '') { $("#v-year").css("border", "2px solid #f00"); }
			      } 
			      else { oldEnough(); 
			      	$("#v-month").css("border", "1px solid #888582");
			      	$("#v-day").css("border", "1px solid #888582");
			      	$("#v-year").css("border", "1px solid #888582");
			      	}
			    }
			    ...
			    function oldEnough() {
			      ...
			      $("#menor").addClass('d-none');
					...
					if (oldEnough) 
			      {
					setCookie('promoJB_age', age.year + "-" + age.month + "-" + age.day, 63);
					if($('#rem').is(':checked')==true)
			        	setCookie('promoJB_remember', 'RememberAge', 63);
			        
			        if ($('#ck').prop('checked')) {
						   setCookie('promoJB_cookies', 'Accept', 63);
						   $('#ageModal').modal('hide');
						}
						else
						{
							$("#nck").removeClass('d-none');
  			...
  			$( ".start" ).click(function() { 
			  		$("#id_tienda option:selected").each(function () {
			           
			            $.post("php/consulta_tiendas.php", {}, function(data){
			                $("#id_tienda").html(data);
			            });			
			        });
			  
			  
			  if(getCookie('promoJB-24hr')=="")
			  {	$('form-reg').trigger("reset"); 
			  	$("#regModal").modal('show');
			  }
			  else
			  {	console.log("Debes esperar 24 horas para volver a participar.");
			...
			function isNumber(e)
			{
				var i = (e = e || window.event).which ? e.which : e.keyCode;
				return !(i > 31 && (i < 48 || i > 57));
			}
